...
export const PortalD = () => {
    const {state, dispatch} = useContext(StoreCtx);
    const {leadsOrOrderCmpName, userPreferences, isReset} = state;

    const defaultBlocks = useMemo(
        () => ['BlocksGridRow', 'AccountProfile', leadsOrOrderCmpName, 'ProductCard', 'Ads', 'MultiChart', 'Chart'],
        [leadsOrOrderCmpName]
    );

    const [blockItems, setBlockItems] = useState(userPreferences?.blocks?.PortalD?.main ||
        defaultBlocks
    );
