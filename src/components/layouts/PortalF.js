...
export const PortalF = () => {
    const {state, dispatch} = useContext(StoreCtx);
    const {leadsOrOrderCmpName, userPreferences, isReset} = state;

    const defaultBlocks = useMemo(
        () => ['BlocksGridRow', 'AccountProfile', 'Chart', leadsOrOrderCmpName, 'ProductCard', 'Ads', 'MultiChart'],
        [leadsOrOrderCmpName]
    );

    const [blockItems, setBlockItems] = useState(userPreferences?.blocks?.PortalF?.main ||
        defaultBlocks
    );
